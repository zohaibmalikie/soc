<script lang="ts">
  ...
  // Svelte stores
  let soulInput = writable('');
  let kcalOutput = writable('');
...
export async function startSwap() {
    try {
      ...
      const userAmount = new BigNumber($soulInput).shiftedBy(decimals).toFixed();
  ...
  <main>
    ...
    <div class="swap-container">
      <h1>SWAP SOUL to KCAL</h1>
      <input class="address-input" type="number" bind:value={$soulInput} placeholder="SOUL Input" />
      <input class="address-input" type="number" bind:value={$kcalOutput} placeholder="KCAL Output" />
