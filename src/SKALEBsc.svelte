<script lang="ts">
  ...
  const BRIDGE_ADDRESS = "0x7f668946770215b0367c258Dc5D27F7b0FC1a36a";
  const TOKEN_ADDRESS = "0x2B06EA3f83aF3AD77140d3976cD1dE46443Fa5c6";
   ...
   // Importing ABI of the contracts
    ...
    async function transferEth() {
      ...
      try {
        ...
        //string memory to, uint amount, uint nonce, bytes calldata signature, uint chainId

        const signature = await signer.signMessage(toBeArray(message));
  
        // Calling the function of the BridgeEth contract
        const tx = await bridgeContract.burn(PHAAddress, amountInWei, nonce, signature, chainId);
  
        //Listening the events emited in eth side
        bridgeContract.on("Transfer", (from, to, amount, date, nonce, signature, step, chainId) => {
          let data = {
            from, to, amount, date, nonce, signature, step, chainId
          };
  ...
  <main>
    <h1 class="mb-10">SKALE To BSC Bridge</h1>
  
    {#if $EthereumAddress}
      <div class="address-container">
        <p>Connected to SKALE <br /><b>{$EthereumAddress}</b></p>
      </div>
      <div class="address-container">
